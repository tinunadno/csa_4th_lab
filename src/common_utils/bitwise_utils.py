def get_signal_bit(signal: list[int], position: int) -> int:
    ...
    word_index: int = position // 32
    bit_offset: int = position % 32
...
def set_signal_bit(signal: list[int], position: int, val: int) -> None:
    if position < 0:
        raise IndexError("Position must be non-negative")
...
def get_int_cut(src: int, pos: list[int]) -> int:
    if len(pos) == 1:
