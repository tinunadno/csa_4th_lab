...
class Define:
    def __init__(self, lines: list[str], idx: int):
        ...
        if "{" not in lines[idx] and "(" not in lines[idx]:
            ...
            if idx == len(lines):
                raise SyntaxError(
                    f"no opening bracket in define: {lines[idx_macro_start]}"
                )
            if lines[idx].strip() != "{":
            ...
            if idx == len(lines):
                raise SyntaxError(
                    f"no closing brackets in define: {lines[idx_macro_start]}"
                )
            ...
            for i in range(idx_macro_start, idx + 1):
                if "}" in lines[idx_macro_start]:
                    lines[idx_macro_start] = lines[idx_macro_start][
                        lines[idx_macro_start].find("}") + 1 :
                    ]
                    break
                else:
    ...
    def substitute(self, code: str) -> str:
        ...
        while match:
            ...
            args = (
                macro_usage[macro_usage.find("(") + 1 : macro_usage.find(")")]
                .replace(" ", "")
                .split(",")
            )
            processed_macro = self.body
            for i in range(len(self.args)):
                processed_macro = re.sub(
                    r"\b" + re.escape(self.args[i]) + r"\b",
                    str(args[i]),
                    processed_macro,
                )
            processed_macro = re.sub(
                r"\b" + re.escape("body") + r"\b", str(macro_body), processed_macro
            )
    ...
    while i < current_lines_size:
        ...
        if "#include" in lines[i]:
            ...
            try:
                ...
                raise SyntaxError(
                    "included file doesn't exist: " + lines[i] + "  " + include_path
                )
            lines.pop(i)
