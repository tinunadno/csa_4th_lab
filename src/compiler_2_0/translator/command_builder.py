...
def get_replacement(token: str, arg_desc: str) -> list:  # type: ignore
    ...
    while token_index < token_len:
        if arg_desc_index >= arg_desc_len:
            raise SyntaxError(
                f"got bad replacement's params, token: {token}, desc: {arg_desc}"
            )
        if arg_desc[arg_desc_index] != token[token_index]:
            ...
            while token_index < token_len:
                if (not is_border_replacement) and token[
                    token_index
                ] == repl_end_symbol:
                    break
            ...
            if is_border_replacement:
                if replacement_index != replacement_count:
                    raise SyntaxError(
                        f"expected more substitution arguments, token: {token}, desc: {arg_desc}"
                    )
                return replacements
        arg_desc_index += 1
        token_index += 1
...
def build_command(
    ...
    if dec_rule == {}:
        raise SyntaxError(
            f"Can't find this mnemonic in internal config: {mnemonic} in instruction {instruction}"
        )
    ...
    if type_desc == {}:
        raise SyntaxError(
            f"found command description with invalid type: {instruction}, description: {dec_rule}"
        )
    type_desc = type_desc["bit_layout"]
    translated_instruction = 0
    try:
        translated_instruction = set_int_cut(
            translated_instruction, type_bits, int(c_type)
        )
    except Exception:
        raise SyntaxError(
            f"invalid type in commands descriptions: type: {c_type}, instruction: {instruction}"
        )
    ...
    try:
        ...
        translated_instruction = set_int_cut(
            translated_instruction, funct_bits, funct_val
        )
    except ValueError as e:
        raise SyntaxError(str(e))
    except Exception:
        raise SyntaxError(
            f"invalid functional bits found in the config: {dec_rule['functional_bits_match']}"
        )
    ...
    for i in range(len(command_arguments)):
        ...
        translated_instruction = get_replacement_substitution_rules(
            replacement,
            command_arguments[i][1:],
            type_desc,
