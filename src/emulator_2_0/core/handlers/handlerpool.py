...
def do_data_forward(
    ...
    tick_logs.append(
        f"[{stage}] Current forwarding head: reg[{current_forward[0]}] = 0x{current_forward[1]:X}"
    )
...
def get_data_forward(
    ...
    if current_forward[0] == r_dest and current_forward[2] == 1:
        tick_logs.append(
            f"f[{stage}] Got forward {current_forward[1]} for reg[{r_dest}]"
        )
...
class MemHandler(Handler):
    def handle(self, args: list[object], is_valid: bool, tick_logs: list[str]) -> bool:
        ...
        mem_signals: PipelineSignal = args[2]  # type: ignore
        df_signals: PipelineSignal = args[3]  # type: ignore
        wb_signal: PipelineSignal = args[4]  # type: ignore
        alu_output: PipelineSignal = args[5]  # type: ignore
        ...
        tick_logs.append(
            f"[MEM] Address: 0x{address:X}, Operation: {'WRITE' if need_write else 'READ'}, "
        ...
        if need_write:
            ...
            if write_byte:
                tick_logs.append(
                    f"[MEM] Writing byte: 0x{value & 0xFF:02X} to 0x{address:X}"
                )
        ...
        tick_logs.append(
            f"[MEM] Current forwarding head: reg[{current_forward[0]}] = 0x{current_forward[1]:X}"
        )
...
class InstructionLoadHandler(Handler):
    ...
    def handle(
        self, args: list[object | int], is_valid: bool, tick_logs: list[str]
    ) -> bool:
        tick_logs.append("[IF] Started instruction fetch stage")
        regs: Registers = args[1]  # type: ignore
        ir_reg_name = args[4]
        stall_signal: PipelineSignal = args[2]  # type: ignore
        int_controller: InterruptionController = args[6]  # type: ignore
        if int_controller.is_interruption():
            tick_logs.append(
                f"[IF] got an interruption on tick: {int_controller.current_tick}"
            )
        ...
        if len(self.inserted_commands) != 0:
            ...
            tick_logs.append(
                f"[IF] left interruption instructions: {self.inserted_commands}"
            )
            return True
        pc_reg_name = args[3]
...
class InstructionDecoderHandler(Handler):
    ...
    def handle(self, args: list[object], is_valid: bool, tick_logs: list[str]) -> bool:
        ...
        regs: Registers = args[1]  # type: ignore
        ir_reg_name = args[2]
        command = regs.get_reg(ir_reg_name)  # type: ignore
                    ...
                    if reg_arg not in cmd_desc["bit_layout"]:
                        if reg_arg.isdigit():
                            ...
                            if regs.is_common(reg_arg):
                                ...
                                value = get_data_forward(
                                    df_signals, reg_num, value, "ID", tick_logs
                                )
                    else:
                        reg_arg_bits = cmd_desc["bit_layout"][reg_arg]["bits"]
                        value = get_int_cut(command, reg_arg_bits)
                        if reg_arg == "imm" and (
                            "dont_cast_my_immediate" not in c_desc
                        ):
                        ...
                        if reg_arg != "imm" and not need_reg_num:
                            ...
                            tick_logs.append(
                                f"[ID] Got register {reg_arg} value: {value}"
                            )
                    signals[signal_name].set_signal(sig_arg, value)
                    tick_logs.append(
                        f"[ID] Set signal {signal_name}.{sig_arg} = {value}"
                    )
                else:
...
class AluExecutionHandler(Handler):
    def __init__(self) -> None:
        self.flags = {"N": False, "Z": False, "V": False, "C": False}
        self.last_result = 0
    ...
    def _update_flags(
        ...
        if len(operands) != 0:
            ...
            self.flags["C"] = (result >> 32) & 1  # type: ignore
    ...
    def handle(self, args: list[object], is_valid: bool, tick_logs: list[str]) -> bool:
        ...
        wb_signals: PipelineSignal = args[2]  # type: ignore
        df_signals: PipelineSignal = args[3]  # type: ignore
        mem_signals: PipelineSignal = args[4]  # type: ignore
        ...
        signals = {
            ...
            "FORCE_DISCARD_FORWARDING": ex_signal.get_signal(
                "FORCE_DISCARD_FORWARDING"
            ),
        ...
        need_forwarding = (
            (need_mem != 1)
            and (need_wb == 1)
            and (signals["FORCE_DISCARD_FORWARDING"] != 1)
        )
        ...
        if signals["set_nzvc"] != 0:
            ...
            tick_logs.append(
                f"[EX] setting nzvc: {self.flags['N'], self.flags['Z'], self.flags['V'], self.flags['C']}"
            )
            return True
        ...
        if signals["get_nzvc"] != 0:
            ...
            tick_logs.append(
                f"setted this value to ALU output signal: {self._to_unsigned32(self.get_nzvc())}"
            )
            do_data_forward(
                df_signals,
                df_destination,
                self._to_unsigned32(self.get_nzvc()),
                "EX",
                tick_logs,
            )
            return True
        ...
        try:
            if signals["add"]:
                reg2 = (
                    -self._to_signed32(reg2)
                    if signals["neg_second"]
                    else self._to_signed32(reg2)
                )
                ...
                if signals["cycl"]:
                    if signals["sh_dir"]:
                        result = (
                            (reg1 << shift_amount) | (reg1 >> (32 - shift_amount))
                        ) & 0xFFFFFFFF
                    else:
                        result = (
                            (reg1 >> shift_amount) | (reg1 << (32 - shift_amount))
                ...
                if condition_met:
                    ...
                    tick_logs.append(
                        f"[EX] Branch taken: PC = {reg1} + {reg2} = {result}"
                    )
                else:
            ...
            if not signals["discard_nzvc"]:
                ...
                tick_logs.append(
                    f"[EX] Operation completed (flags discarded). Result: {result}"
                )
                return True
...
class RegisterHandler(Handler):
    def handle(self, args: list[object], is_valid: bool, tick_logs: list[str]) -> bool:
        ...
        if need_wb == 0:
            ...
            return True
        alu_output: PipelineSignal = args[2]  # type: ignore
        regs: Registers = args[0]  # type: ignore
...
class HandlerPool:
    def __init__(self, c_types: CommandTypes, command_descs: dict) -> None:  # type: ignore
        self.handlers = {
            ...
            "instruction_decoder_handler": InstructionDecoderHandler(
                c_types, command_descs
            ),
