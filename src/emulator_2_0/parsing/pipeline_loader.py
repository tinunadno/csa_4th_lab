...
def parse_bin_file(
    bin_data: bytearray,
) -> dict[str, int | list[list[tuple[int, bytearray]]] | list[int]]:
    offset = 0
...
def parse_config(
    config_path: str, user_config_path: str, executable_bin_stuff: bytearray
) -> tuple[int, Logger]:
    ...
    int_controller = InterruptionController(
        interruptions, interruption_vector, input_addr, data, data_mem_
    )
    ...
    pl = Pipeline(
        ...
        data["pipeline"]["pipeline_signals"],
        data["instructions"],
        int_controller,
    )
