...
def perform_tick(db_state: DebugState, pl: Pipeline) -> None:
    if db_state.performed_tick:
        ...
        db_state.log_blocks[1] = pl.data_mem.get_memory_view(
            db_state.last_data_mem_cut[0], db_state.last_data_mem_cut[1]
        )
    else:
        db_state.log_blocks = [
            pl.last_tick_logs,
            pl.data_mem.get_memory_view(
                db_state.last_data_mem_cut[0], db_state.last_data_mem_cut[1]
            ),
            pl.inst_mem.get_memory_view(
                db_state.last_inst_mem_cut[0], db_state.last_inst_mem_cut[1]
            ),
        ]
...
class Command(ABC):
    ...
    def print_default(self, db_state: DebugState) -> None:
        ...
        print(
            f"IS INTERRUPTION: {self.pl.get_static_signal('INTERRUPT_signal', 'is_interrupted') != 0}"
        )
...
class MemSlice(Command):
    ...
    def handle(self, cmd: str, db_state: DebugState) -> None:
        ...
        os.system("clear")
        (db_state.last_data_mem_cut[0], db_state.last_data_mem_cut[1]) = list(
            map(int, cmd.split(" ")[1:])
        )
        db_state.log_blocks[1] = self.pl.data_mem.get_memory_view(
            db_state.last_data_mem_cut[0], db_state.last_data_mem_cut[1]
        )
        self.print_default(db_state)
        db_state.performed_tick = True
...
class InstSlice(Command):
    ...
    def handle(self, cmd: str, db_state: DebugState) -> None:
        if not db_state.performed_tick:
            print("PERFORM A TICK FIRST!")
        os.system("clear")
        db_state.last_inst_mem_cut[0], db_state.last_inst_mem_cut[1] = list(
            map(int, cmd.split(" ")[1:])
        )
        db_state.log_blocks[1] = self.pl.inst_mem.get_memory_view(
            db_state.last_inst_mem_cut[0], db_state.last_inst_mem_cut[1]
        )
        self.print_default(db_state)
...
class DecompSlice(Command):
    ...
    def handle(self, cmd: str, db_state: DebugState) -> None:
        ...
        db_state.log_blocks[0] = self.pl.inst_mem.get_decompiled_memory_view(
            db_state.last_inst_mem_cut[0], db_state.last_inst_mem_cut[1]
        )
        self.print_default(db_state)
